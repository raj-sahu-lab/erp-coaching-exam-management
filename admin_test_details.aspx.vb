...
Imports System.Data
Partial Class admin_test_details
    ...
    Protected Sub search_grid_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles search_grid.Click
        ...
        If dd_selection_date.SelectedValue = "" Then
            'MessageBox.Show("Select search criteria !!", "SELECT SEARCH CLAUSE", MessageBoxButtons.OK, MessageBoxIcon.Error)
            'Page.ClientScript.RegisterStartupScript(Type.GetType("System.String"), "ALERT", "alert('Select search criteria');menubar=yes;window.location.href = 'followup.aspx';", True)
            dd_selection_date.Focus()
            Exit Sub
        End If
        Try
            Dim sSQL As String = "SELECT [Mobile], [StudentName], [ExamDate], [Gender], [Degree], [College], [Yearsem], [branch], [CourseTarget], [totalattempts], [correct], [wrong], [Percentage],(CAST(correct AS INTEGER) - (CAST(wrong AS INTEGER)*.25)) as per, [Waiver], [conveyed],  [EnquiryCreated] From [ScholarshipReg] WHERE ([ExamDate] = @enq85)"
            SqlDataSource_students_test.SelectParameters.Add("enq85", dbType:=Data.DbType.Date, value:=dd_selection_date.SelectedItem.Text)
            SqlDataSource_students_test.SelectParameters("enq85").DefaultValue = dd_selection_date.SelectedItem.Text
    ...
    Protected Sub update_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles update.Click
        ...
        For i As Integer = 0 To CoolGridView2.Rows.Count - 1
            ...
            If tBox.Text.Equals("") Then
                Continue For
            Else
                cmd.CommandText = "UPDATE ScholarshipReg SET Waiver = '" & tBox.Text & "' WHERE Mobile = '" & theid & "'"
                cmd.ExecuteNonQuery()
