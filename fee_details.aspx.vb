    ...
    Protected Sub search_grid_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles search_grid.Click
        ...
        Try
            If dd_selection.Text = "Pending Fees" Then
                Dim sSQL As String = "SELECT enquiry_details.student_name,enquiry_details.course_enq,fee_collect.type_fee,fee_collect.rem_fee,fee_collect.mobileno_fee,fee_followup.f_1,fee_followup.date_f_1,fee_collect.due_date1,fee_followup.councellor_f1,fee_followup.f_2 ,fee_followup.date_f_2,fee_collect.due_date2,fee_followup.councellor_f2,fee_followup.f_3 ,fee_followup.date_f_3,fee_followup.councellor_f3,fee_collect.remark,fee_collect.defaulter FROM enquiry_details INNER JOIN fee_collect ON enquiry_details.mobile = fee_collect.mobileno_fee LEFT OUTER JOIN fee_followup ON fee_collect.mobileno_fee = fee_followup.mobileno_followup WHERE((((fee_collect.token_date >= @enq7) AND (fee_collect.token_date <= @enq8)) OR ((fee_collect.pay_date_lumpsum >= @enq7) AND (fee_collect.pay_date_lumpsum <= @enq8)) OR ((fee_collect.pay_date1 >= @enq7) AND (fee_collect.pay_date1 <= @enq8))) AND (fee_collect.rem_fee > '0.0000'))"
                SqlDataSource_fee.SelectParameters.Add("enq7", dbType:=Data.DbType.Date, value:=startdate.Text)
                SqlDataSource_fee.SelectParameters("enq7").DefaultValue = startdate.Text
                SqlDataSource_fee.SelectParameters.Add("enq8", dbType:=Data.DbType.Date, value:=enddate.Text)
                SqlDataSource_fee.SelectParameters("enq8").DefaultValue = enddate.Text
